...
namespace SkyHubTycoon.EditorTools
{
    public static class SkyHubProjectBootstrap
    {
        ...
        public static void CreateFullStarterProject()
        {
            ...
            GameObject passengerPrefab = CreatePassengerPrefab();
            ...
            CreateCamera();
            CreateLight();
            CreateGameSystems(floorArray, buildables, previewPrefab, passengerPrefab, validPreview, invalidPreview, warningPreview);
        ...
        private static GameObject CreatePassengerPrefab()
        {
            string path = PrefabRoot + "/Passenger.prefab";
            GameObject existing = AssetDatabase.LoadAssetAtPath<GameObject>(path);
            if (existing != null) return existing;

            GameObject passenger = GameObject.CreatePrimitive(PrimitiveType.Capsule);
            passenger.name = "Passenger";
            passenger.transform.localScale = new Vector3(0.28f, 0.52f, 0.28f);
            Renderer renderer = passenger.GetComponent<Renderer>();
            renderer.sharedMaterial = CreateMaterial("Passenger_CozyBlue", new Color(0.2f, 0.55f, 1f));
            Object.DestroyImmediate(passenger.GetComponent<Collider>());
            passenger.AddComponent<PassengerAgent>();
            GameObject prefab = PrefabUtility.SaveAsPrefabAsset(passenger, path);
        ...
        private static void CreateGameSystems(FloorDefinition[] floors, BuildableDefinition[] buildables, GameObject previewPrefab, GameObject passengerPrefab, Material validPreview, Material invalidPreview, Material warningPreview)
        {
            ...
            PassengerManager passengerManager = root.AddComponent<PassengerManager>();
            ...
            passengerManager.grid = grid;
            passengerManager.airport = airport;
            passengerManager.alertManager = alerts;
            passengerManager.passengerPrefab = passengerPrefab;

            scheduler.airport = airport;
            scheduler.grid = grid;
            scheduler.passengerManager = passengerManager;
            scheduler.alertManager = alerts;
            alerts.cameraController = cameraController;
