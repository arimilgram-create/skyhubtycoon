...
- Basic airport flow validation for departing flights: Entrance → Check-in → Security → Waiting area → Gate, plus gate/taxiway/runway connectivity.
- Flight scheduling is blocked unless the route validator passes. Missing-route alerts are shown in the runtime UI and clickable alerts can move the camera near the relevant object.
- Basic passenger AI for scheduled flights: passengers spawn at the entrance, pathfind across passenger-safe terminal floors to check-in, wait, path to security, wait, path to waiting seats or gate, board through the gate, then disappear.
- Passenger satisfaction is shown in the UI and can drop when passengers cannot find a route, queues are too long, seats are missing near the gate, or flights are delayed.
...
2. Improve prototype feel before adding passengers:
   ...
   - Persistent on-screen flow status before the player presses Schedule flight.
   - More visible path/connection feedback for gate ↔ taxiway ↔ runway and entrance/check-in/security flow.
   - Better passenger animations, spacing, and queue positions instead of simple overlapping lines.
...
6. Expand passenger agents after the simple prototype loop is stable: clearer boarding timing, visible line positions, and passenger-specific status bubbles.
...
- The first playable route should be: entrance → check-in → security → waiting → gate, plus runway/taxiway for aircraft operations; scheduling must stay disabled or fail with clear alerts until this route is connected, and passengers should only walk on passenger-safe terminal floors.
