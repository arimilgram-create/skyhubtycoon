...
const requiredPaths = [
  ...
  'Assets/Scripts/Simulation/AirportFlowValidator.cs',
  'Assets/Scripts/Simulation/PassengerAgent.cs',
  'Assets/Scripts/Simulation/PassengerManager.cs',
  'Assets/Scripts/Simulation/PassengerPathfinder.cs',
...
for (const marker of ['BuildStartMenu', 'BuildPauseMenu', 'Escape', 'RestartScene', 'ShowMainMenu', 'ScaleWithScreenSize', '1920f, 1080f', 'Schedule flight', 'Passenger satisfaction']) {
...
const flow = readFileSync('Assets/Scripts/Simulation/AirportFlowValidator.cs', 'utf8');
for (const marker of [
  'ValidateBasicDepartingFlight',
  'HasPathBetween',
  'No entrance connected to terminal.',
  'No path from entrance to check-in.',
  'No path from check-in to security.',
  'No path from security to gate.',
  'Gate is missing taxiway connection.',
  'Runway is missing taxiway connection.'
]) {
  assert.equal(flow.includes(marker), true, `airport flow validator should include ${marker}`);
}

const flightScheduler = readFileSync('Assets/Scripts/Simulation/FlightScheduler.cs', 'utf8');
for (const marker of ['AirportFlowValidator', 'ValidateFlow', 'flow.valid', 'PushAlert(issue.message, issue.focusPosition)', 'BeginBoardingFlight']) {
  assert.equal(flightScheduler.includes(marker), true, `flight scheduler should include ${marker}`);
}

const alerts = readFileSync('Assets/Scripts/UI/AlertManager.cs', 'utf8');
for (const marker of ['Button', 'FocusCamera', 'FocusOnWorldPosition', 'click to view']) {
  assert.match(alerts, new RegExp(marker), `alerts should include ${marker}`);
}


const passengerAgent = readFileSync('Assets/Scripts/Simulation/PassengerAgent.cs', 'utf8');
for (const marker of ['ToCheckIn', 'CheckInWait', 'ToSecurity', 'SecurityWait', 'ToWaiting', 'ToGate', 'PassengerBoarded']) {
  assert.equal(passengerAgent.includes(marker), true, `passenger agent should include ${marker}`);
}

const passengerManager = readFileSync('Assets/Scripts/Simulation/PassengerManager.cs', 'utf8');
for (const marker of [
  'BeginBoardingFlight',
  'Passenger cannot find a route.',
  'Check-in line is too long.',
  'Security line is too long.',
  'No seats near the gate.',
  'Flight delayed.',
  'AdjustPassengerSatisfaction'
]) {
  assert.equal(passengerManager.includes(marker), true, `passenger manager should include ${marker}`);
}

const passengerPathfinder = readFileSync('Assets/Scripts/Simulation/PassengerPathfinder.cs', 'utf8');
for (const marker of ['TryFindPath', 'IsPassengerSafeFloor', 'ZoneType.Airfield', 'ZoneType.Staff', 'ZoneType.Baggage', 'AgentPathType.Passenger']) {
  assert.equal(passengerPathfinder.includes(marker), true, `passenger pathfinder should include ${marker}`);
}
...
for (const marker of [
  ...
  assert.equal(placement.includes(marker), true, `placement validator should include warning: ${marker}`);
}
